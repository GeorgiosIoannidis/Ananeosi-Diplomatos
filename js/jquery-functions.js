$("document").ready(function () {
  ...
  function convertURLsToLinks(text) {
    return text.replace(
      /https:\/\/www\.pkm.gov\.gr\/[\S]+/g,
      '<a href="$&" target="_blank">' + "Διεύθυνση Μεταφορών" + "</a>" + ""
    );
  }
  ...
  function skipToEnd(message) {
    ...
    const error =
      currentLanguage === "greek"
        ? "Λυπούμαστε αλλά δεν δικαιούστε ανανέωση διπλώματος!"
        : "We are sorry but you are not entitled to the driving license renewal!";
  ...
  function retrieveAnswers() {
    ...
    if (allAnswers[5] === "1" || allAnswers[5] === "2") {
      ...
      currentLanguage === "greek"
        ...
        : setResult(
            "The companion is also entitled with the same transportation card."
          );
    }
  ...
  function submitForm() {
    ...
    currentLanguage === "greek"
      ? $(".question-container").append(
          "<br /><br /><h5 class='answer'>Τα δικαιολογητικά που πρέπει να προσκομίσετε για να λάβετε την ανανέωση του διπλώματoς σας:</h5><br />"
        )
      : $(".question-container").append(
          "<br /><br /><h5 class='answer'>The documents you need to provide in order to receive your driving license renewal:</h5><br />"
        );
  ...
  $("#nextQuestion").click(function () {
    if ($(".govgr-radios__input").is(":checked")) {
      ...
      if (currentQuestion === 0 && selectedRadioButtonIndex === 1) {
        currentQuestion = -1;
        currentLanguage === "greek"
          ? skipToEnd("Το δίπλωμά σας θα πρέπει να έχει λήξει.")
          : skipToEnd("Your license must have expired.");
      } else if (currentQuestion === 1 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek"
          ? skipToEnd(
              "Δεν έχετε δικαίωμα για ανανέωση διπλώματος, για λόγους υγείας."
            )
          : skipToEnd(
              "You are not eligible for a license renewal for health reasons."
            );
      } else if (currentQuestion === 2 && selectedRadioButtonIndex === 1) {
        currentQuestion = -1;
        currentLanguage === "greek"
          ? skipToEnd("Πρέπει να έχετε κάτω από 50 πόντους ποινής.")
          : skipToEnd("You must have fewer than 50 penalty points.");
      } else if (currentQuestion === 3 && selectedRadioButtonIndex === 3) {
        currentQuestion = -1;
        currentLanguage === "greek"
          ? skipToEnd(
              "Πρέπει να είστε κάτοικος Ελλάδας για τουλάχιστον 6 μήνες."
            )
          : skipToEnd(
              "You must be a resident of Greece for at least 6 months."
            );
      } else if (currentQuestion === 4 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek"
          ? skipToEnd("Πρέπει να είστε εγγεγραμμένοι στο TaxisNet.")
          : skipToEnd("You must be registered with TaxisNet.");
      } else {
